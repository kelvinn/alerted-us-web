...
# TODO
"""
class AlertLocationSearchAPI(APIView):
    ...
    def get(self, request, alert_id, format=None):
        ...
        for info in alert.info_set.all():
            for area in info.area_set.all():
                if area.geom:  # We do this to prevent tests from bombing out...
                    locations = Location.objects.filter(geom__intersects=area.geom)
                    for loc in locations:
                        notif = Notification(cap_area=area, location=loc)

                        # TODO send out actual notifications

                        notif.message_id = 1
                        notif.save()
                        msg = "At least one match"
        return Response(msg)

    def post(self, request, format=None):

        #user_serializer = User.objects.get(username=request.user)
